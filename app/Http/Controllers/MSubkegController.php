...
class MSubkegController extends Controller
{
    public function show(Request $request)
    {
        ...
        if($tahun && $idKab && $idOpd && $idUnit){
            $m_subkeg = UnitSubKeg::with(['subKegiatan'])
            ->where(['id_kab' => $idKab,
                                'id_opd' => $idOpd,
                                'id_unit' => $idUnit,
        ...
        $m_subkegResource = MSubKegResource::collection($m_subkeg);
    ...
    public function index( Request $request )
    {
        ...
        $kegiatanQuery = MKeg::where('m_kegs.tahun', $tahun_aktif->keg)
            ->select(
        ...
        $kegiatan = MKeg::where('m_kegs.tahun', $tahun_aktif->keg)
                    ->when($search, function($query) use ($search) {
                        $query->where('nama', 'LIKE', "%{$search}%")
                            ->orWhere('id_keg', 'like', "%{$search}%");
                    });
        // Query untuk Sub Kegiatan
        $subkegiatanQuery = MSubkeg::where('m_subkegs.tahun', $tahun_aktif->keg)
            ->with('keg')
    ...
    public function store(Request $request)
{
    ...
    public function update(Request $request, MSubkeg $subkegiatan)
{
    ...
    public function destroy(MSubkeg $subkegiatan, $id)
    {
