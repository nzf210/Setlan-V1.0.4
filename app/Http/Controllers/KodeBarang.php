...
use App\Models\KodeBarang as KdBarang;
use Illuminate\Support\Facades\Cookie;
use Illuminate\Support\Facades\Gate;
use Illuminate\Validation\Rule;
use Inertia\Inertia;

class KodeBarang extends Controller
{
    public function get(Request $request){
        $idKab = Cookie::get('id_kab');
        $tahun = Cookie::get('year');
        $tahun_aktif = Year::where('year', $tahun)->first();
        $query = KdBarang::whereRaw('LENGTH(id_kd_barang) = 20')->where(['id_kab' => $idKab, 'tahun' => $tahun_aktif->kd_id_barang]);
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Setlan/Barang/KodeBarang/Index', [
            ...
            'can' => [
                'create' => auth()->user()->can('create', KodeBarang::class),
                'edit' => auth()->user()->can('update', KodeBarang::class),
                'delete' => auth()->user()->can('delete', KodeBarang::class),
            ],
        ]);
    }
    ...
    public function store(Request $request)
{
    ...
    public function update(Request $request, KdBarang $kodeBarang)
{
    // Authorization check
    ...
    public function destroy(KdBarang $kdBarang, $id)
    {
